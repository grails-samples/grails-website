...
<div id="content" class="content-aside-2" role="main">
    ...
    <section id="downloadArchives">
        ...
        <div class="majorVersion">
            <h3>Grails ${majorVersion}</h3>
            <g:each var="download" in="${groupedDownloads[majorVersion]}">
            <div class="archiveDetails">
                <h4>${download.download.softwareVersion}</h4>
                <p><a href="${download.download?.releaseNotes}">Release Notes</a></p>
                <ul class="links">
                    <li><a href="${download.binary[0].mirrors[0].urlString}">Binary</a></li>
                    <g:if test="${download.source[0]?.mirrors}">
                    <li><a href="${download.source[0].mirrors[0].urlString}">Source</a></li>
                    </g:if>
                    <g:if test="${download.documentation[0]?.mirrors}">
                    <li><a href="${download.documentation[0].mirrors[0].urlString}">Docs</a></li>
                    </g:if>
                </ul>
            </div>
            </g:each>
    ...
    <section id="ubuntuInstall">
        ...
        <!-- Replaced by the GitHub snippet if JavaScript is enabled -->
        <pre class="nojs">
